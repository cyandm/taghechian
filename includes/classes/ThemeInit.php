...
class ThemeInit
{
	...
	public static function frontendEnqueueScripts()
	{
		...
		$bundle_css_path = self::$isDev ? '/js/dist/cyn-theme-script.bundle.css' : '/js/dist/cyn-theme-script.bundle.min.css';

		//Enqueue styles
		wp_enqueue_style(THEME_SLUG, THEME_ASSETS_URI . $css_path, [], self::$version);

		//Enqueue bundle styles (Plyr, etc.)
		wp_enqueue_style(THEME_SLUG . '-bundle', THEME_ASSETS_URI . $bundle_css_path, [], self::$version);
